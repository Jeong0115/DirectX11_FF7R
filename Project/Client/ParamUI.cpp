...
{// 정적맴버 초기화
    ...
    int ParamUI::Param_Tex(const string& _strDesc, Ptr<Texture>& _Tex, UI* _UI, UI_DELEGATE_1 _Func)
    {
        ImGui::Text(_strDesc.c_str());
        ...
        if (ImGui::Button(strBtnName.c_str(), ImVec2(18, 18)))
        {
            ...
            pListUI->Reset(SearchListUI::SearchListType::TEXTURES, ImVec2(300.f, 500.f));
    ...
    string ParamUI::GetNextName(const string& _ShowName)
    {
        ...
        char szID[50] = "##";
        char* dest = szID;
        _itoa_s(Next_ID++, dest + 2, 50 - 2, 10);
